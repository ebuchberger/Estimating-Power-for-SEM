...
plot_load_model3 <- results_load_model3 %>%
  ...
  xlab('Sample size\n') + ylab('Power (%)') + 
  ggtitle('Inter-factor correlation = 0.3* \n') +
...
plot_load_model3_highcov <- results_load_model3_highcov %>%
  ...
  xlab('Sample size\n') + ylab('Power (%)') + ggtitle('Inter-factor correlation = 0.7\n') +
  scale_color_discrete('Loading \nstrength') +
  scale_x_continuous(
    expand = c(0, 0),
    limits = c(50, 200),
...
joined_plot <-
  ggarrange(plot_cov_model3_lowload,
            plot_cov_model3,
            plot_load_model3,
